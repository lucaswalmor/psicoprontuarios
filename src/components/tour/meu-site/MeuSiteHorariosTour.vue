...
/** Ordem igual à lista em HorariosTab.vue */
const DIAS = [
...
function buildHorarioSteps() {
    const list = [];
    for (const d of DIAS) {
        const n = d.dia_semana;
        list.push({
            attachTo: { element: `[data-tour="tour-meusite-horario-${n}-switch"]` },
            content: {
                title: `${d.nome} — ativo`,
                description:
                    'Interruptor liga ou desliga este dia na landing. Desligado: o dia não aparece como disponível; os horários ficam esmaecidos e bloqueados.'
            },
            options: popperBottomStart
        });
        list.push({
            attachTo: { element: `[data-tour="tour-meusite-horario-${n}-inicio"]` },
            content: {
                title: `${d.nome} — início`,
                description:
                    'Hora de abertura (formato HH:mm). Ao sair do campo, o texto é normalizado. Só editável quando o dia está ativo.'
            },
            options: popperBottomStart
        });
        list.push({
            attachTo: { element: `[data-tour="tour-meusite-horario-${n}-fim"]` },
            content: {
                title: `${d.nome} — fim`,
                description: 'Hora de encerramento do atendimento neste dia. Deve ser posterior ao início na prática do seu consultório.'
            },
            options: popperBottomStart
        });
    }
    list.push({
        attachTo: { element: '[data-tour="tour-meusite-horarios-salvar"]' },
        content: {
            title: 'Salvar horários',
            description: 'Envia todos os dias (ativo/inativo e faixas) para o servidor; só os dias ativos são exibidos no site público.'
        },
        options: popperBottomStart
    });
    return list;
...
const STEPS = buildHorarioSteps();

export default {
    name: 'MeuSiteHorariosTour',
    components: { VOnboardingWrapper, VOnboardingStep },
    data() {
        return {
            dismissed: false,
            steps: STEPS,
            firstTargetSelector: '[data-tour="tour-meusite-horario-1-switch"]',
            wrapperOptions: {
