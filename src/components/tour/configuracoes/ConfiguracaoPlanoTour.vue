...
const STORAGE_KEY = 'psico_prontuario_tour_config_plano_v1';
...
const STEPS_FULL = [
    {
        attachTo: { element: '[data-tour="tour-config-plano-hero"]' },
        content: {
            title: 'Seu plano e status',
            description:
                'Resumo do nome do plano, valor mensal e etiqueta de status (trial, ativa, pausada etc.). Alertas no topo podem avisar sobre trial, fatura pendente ou cartão em atraso — nesses casos use Alterar cartão nas Ações quando aparecer.'
        },
        options: popperBottomStart
    },
    {
        attachTo: { element: '[data-tour="tour-config-plano-cards"]' },
        content: {
            title: 'Datas, pagamento e ações',
            description:
                'Três blocos: datas importantes (trial, cobrança), informações sobre cobrança via Asaas e botões Alterar cartão, Parar assinatura ou Reativar conforme o estado. Abaixo, lembrete de que parar a assinatura encerra o acesso ao app até reativar.'
        },
        options: popperBottom
    },
    {
        attachTo: { element: '[data-tour="tour-config-plano-faturas"]' },
        content: {
            title: 'Cobranças registradas',
            description:
                'Histórico de faturas sincronizado com a Asaas: vencimento, valor, status e data de pagamento. Use para conferir confirmações e pendências.'
        },
        options: popperBottom
    }
];
...
const STEPS_SIMPLES = [
    {
        attachTo: { element: '[data-tour="tour-config-plano-root"]' },
        content: {
            title: 'Aba Plano',
            description:
                'Aqui você vê o status da sua conta: conta vitalícia sem cobrança recorrente, ou convite para assinar um plano pago com redirecionamento para a página de planos. Com assinatura ativa, o painel completo com cartão, faturas e ações aparece automaticamente.'
        },
        options: popperBottomStart
    }
];
...
export default {
    name: 'ConfiguracaoPlanoTour',
    components: { VOnboardingWrapper, VOnboardingStep },
    props: {
        modo: {
            type: String,
            required: true,
            validator: (v) => v === 'full' || v === 'simples'
        }
    },
    data() {
        return {
    ...
    computed: {
        steps() {
            return this.modo === 'full' ? STEPS_FULL : STEPS_SIMPLES;
        },
        firstTargetSelector() {
            return this.modo === 'full'
                ? '[data-tour="tour-config-plano-hero"]'
