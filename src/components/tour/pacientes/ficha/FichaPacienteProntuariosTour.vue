...
const STORAGE_KEY = 'psico_prontuario_tour_ficha_paciente_tab_prontuarios_v1';
...
export default {
    name: 'FichaPacienteProntuariosTour',
    ...
    computed: {
        steps() {
            const steps = [];
            if (this.temProntuarios) {
                steps.push(
                    {
                        attachTo: { element: '[data-tour="tour-ficha-pront-exportar"]' },
                        content: {
                            title: 'Exportar todos',
                            description:
                                'Gera e baixa a exportação de todos os prontuários deste paciente em arquivo, útil para backup, auditoria ou envio externo.'
                        },
                        options: popperBottomStart
                    },
                    {
                        attachTo: { element: '[data-tour="tour-ficha-pront-relatorio-ia"]' },
                        content: {
                            title: 'Gerar relatório com I.A',
                            description:
                                'Consolida os prontuários em um relatório analítico gerado por inteligência artificial (resumo, padrões, evolução). Plano PRO ou Vitalício: uso liberado; nos demais planos o recurso pode exibir selo PRO e abrir fluxo de upgrade.'
                        },
                        options: popperBottomStart
                    }
                );
            }
            if (this.podeCriarProntuario) {
                steps.push({
                    attachTo: { element: '[data-tour="tour-ficha-pront-novo"]' },
                    content: {
                        title: 'Novo prontuário',
                        description:
                            'Abre o cadastro de um novo registro de prontuário para esta sessão ou evolução clínica. Após salvar, o item aparece na lista abaixo.'
                    },
                    options: popperBottomStart
                });
            }
            steps.push({
                attachTo: { element: '[data-tour="tour-ficha-pront-lista"]' },
                content: {
                    title: 'Lista de prontuários',
                    description:
                        'Todos os prontuários do paciente com paginação. Clique em uma linha para abrir, ler e editar o conteúdo conforme sua permissão.'
                },
        ...
        firstTargetSelector() {
            if (this.temProntuarios) return '[data-tour="tour-ficha-pront-exportar"]';
            if (this.podeCriarProntuario) return '[data-tour="tour-ficha-pront-novo"]';
            return '[data-tour="tour-ficha-pront-lista"]';
        }
    },
    mounted() {
