...
const STORAGE_KEY = 'psico_prontuario_tour_ficha_paciente_tab_sessoes_v1';
...
export default {
    ...
    data() {
        return {
            firstTargetSelector: '[data-tour="tour-ficha-sessoes-periodo"]',
            wrapperOptions: {
    ...
    computed: {
        steps() {
            const steps = [
                {
                    attachTo: { element: '[data-tour="tour-ficha-sessoes-periodo"]' },
                    content: {
                        title: 'Período (mês)',
                        description:
                            'Use as setas para ir ao mês anterior ou ao próximo mês. O texto no meio mostra o mês e o ano da listagem — as sessões exibidas na tabela são sempre desse período.'
                    },
                    options: popperBottomStart
                }
            ];
            if (this.mostrarNovaSessao) {
                steps.push({
                    attachTo: { element: '[data-tour="tour-ficha-sessoes-nova"]' },
                    content: {
                        title: 'Nova sessão',
                        description:
                            'Abre o cadastro de um novo agendamento para este paciente (data, hora e demais dados do agendamento). Se o plano estiver pausado, este botão fica oculto.'
                    },
                    options: popperBottomStart
                });
            }
            steps.push({
                attachTo: { element: '[data-tour="tour-ficha-sessoes-tabela"]' },
                content: {
                    title: 'Lista de sessões',
                    description:
                        'Sessões do mês selecionado com data, hora e ações para editar ou excluir cada agendamento. Use a paginação na parte inferior se houver muitos registros.'
                },
