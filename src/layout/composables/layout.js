...
import { saveThemeConfig, loadThemeConfig, applyThemeConfig } from '@/utils/themeStorage';
...
// Função para obter extensão do preset baseada na cor primária
function getPresetExt(primaryColor) {
...
function applyCompleteTheme(config) {
    try {
        // Aplicar preset
        const presetValue = presets[config.preset];
        if (!presetValue) {
            console.warn(`Preset ${config.preset} não encontrado, usando Aura`);
            config.preset = 'Aura';
        }

        // Aplicar superfície
        const surfacePalette = surfaces.find((s) => s.name === config.surface)?.palette;
        
        // Aplicar tema
        $t().preset(presetValue).preset(getPresetExt(config.primary)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
...
export function useLayout() {
    // Função para inicializar configurações do localStorage
    const initializeConfig = () => {
        const storedConfig = loadThemeConfig();
        if (storedConfig) {
            // Aplicar configurações salvas
            Object.assign(layoutConfig, storedConfig);
            
            // Aplicar tema escuro se necessário
            applyThemeConfig(layoutConfig);
            
            // Aplicar tema completo do PrimeVue
            applyCompleteTheme(layoutConfig);
        }
    };
    ...
    const executeDarkModeToggle = () => {
        ...
        saveCurrentConfig(); // Salvar após alteração
    };
    ...
    // Inicializar configurações quando o composable for usado
    onMounted(() => {
        initializeConfig();
    });
