            ...
            <div
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-3">
                        ...
                        <span class="text-muted-color font-medium">{{ stepSubtitle }}</span>
                    </div>
                    ...
                    <div v-if="currentStep === 1">
                        ...
                        <InputText id="email" type="email" placeholder="Digite seu email" class="w-full md:w-[30rem] mb-4"
                            v-model="email" @keyup.enter="sendCode" />

                        <div class="flex gap-3">
                            <Button type="button" label="Voltar" class="flex-1" @click="goBack" :disabled="isLoading" severity="secondary">
                                <i class="pi pi-arrow-left mr-2"></i>
                            </Button>
                            <Button type="button" label="Enviar Código" class="flex-1" @click="sendCode" :disabled="isLoading || !email">
                    ...
                    <div v-if="currentStep === 2">
                        ...
                        <div class="flex justify-center mb-6">
                            <InputOtp v-model="otpCode" :length="6" />
                        </div>

                        <div class="flex gap-3">
                            <Button type="button" label="Voltar" class="flex-1" @click="previousStep" :disabled="isLoading" severity="secondary">
                                <i class="pi pi-arrow-left mr-2"></i>
                            </Button>
                            <Button type="button" label="Verificar Código" class="flex-1" @click="verifyCode" :disabled="isLoading || otpCode.length !== 6">
                                <ProgressSpinner style="width: 20px; height: 20px" strokeWidth="8" fill="transparent"
                                    animationDuration=".5s" aria-label="Custom ProgressSpinner" v-if="isLoading" />
                            </Button>
                        </div>
                    ...
                    <div v-if="currentStep === 3">
                        <label for="newPassword"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Nova Senha</label>
                        <Password id="newPassword" v-model="newPassword" placeholder="Nova senha" :toggleMask="true"
                            class="mb-4" fluid :feedback="true"></Password>

                        <label for="confirmPassword"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Confirmar Nova Senha</label>
                        <Password id="confirmPassword" v-model="confirmPassword" placeholder="Confirme a nova senha" :toggleMask="true"
                            class="mb-4" fluid :feedback="false" @keyup.enter="changePassword"></Password>

                        <div class="flex gap-3">
                            <Button type="button" label="Voltar" class="flex-1" @click="previousStep" :disabled="isLoading" severity="secondary">
                                <i class="pi pi-arrow-left mr-2"></i>
                            </Button>
                            <Button type="button" label="Alterar Senha" class="flex-1" @click="changePassword" :disabled="isLoading || !isFormValid">
                                <ProgressSpinner style="width: 20px; height: 20px" strokeWidth="8" fill="transparent"
                                    animationDuration=".5s" aria-label="Custom ProgressSpinner" v-if="isLoading" />
                            </Button>
                        </div>
...
export default {
    ...
    computed: {
        ...
        stepSubtitle() {
            switch (this.currentStep) {
                case 1: return 'Digite seu email para receber o código de verificação';
                case 2: return 'Digite o código enviado para seu email';
                case 3: return 'Digite sua nova senha';
                default: return 'Digite seu email para receber o código de verificação';
            }
        },
        isFormValid() {
        ...
        async verifyCode() {
            if (this.otpCode.length !== 6) {
                this.$toast.add({
                    severity: 'error',
                    summary: 'Erro',
                    detail: 'Por favor, digite o código completo',
                    life: 3000
                });
                return;
            }
